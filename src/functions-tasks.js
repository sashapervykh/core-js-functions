...
function logger(func, logFunc) {
  return function log(...rest) {
    const restString = rest.map((elem) => JSON.stringify(elem)).join(',');
    logFunc(`${func.name}(${restString}) starts`);
    const result = func(...rest);
    logFunc(`${func.name}(${restString}) ends`);
    return result;
  };
}
